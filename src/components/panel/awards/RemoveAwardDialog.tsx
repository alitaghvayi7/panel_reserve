...
const RemoveAwardDialog = ({ date, id, description, title }: TProps) => {
  const router = useRouter();
  const session = useSession();
  const closeButtonRef = useRef<null | HTMLButtonElement>(null);
  const handleSubmit = async (e: any) => {
    e.preventDefault();
    const req = await fetch(`/api/awards/deleteAward`, {
      method: "POST",
      headers: {
        ...
        <DialogPortal>
          ...
          <DialogContent className="max-w-[308px] sm:max-w-[390px] rounded-2xl lg:max-w-[484px] px-4 lg:px-12">
            ...
            >
              ...
              <p>آیا اطمینان دارید که {title} حذف شود؟</p>
              <div className="flex flex-col items-stretch gap-4 self-start">
              ...
              <div className="grid grid-cols-2 grid-rows-1 gap-4 w-full px-10">
                <DialogClose
                  ref={closeButtonRef}
...
export default RemoveAwardDialog;
