...
import {
  TUserFilterMenuItem,
  UserFilterQueryNames,
} from "@/types/FilterTickets";

const PanelFilterUsers = ({ menus }: { menus: TUserFilterMenuItem[] }) => {
  const router = useRouter();
  const [openMenu, setOpenMenu] = useState<"ادمین" | null>(null);
  const [isResetFormDisabled, setIsResetFormDisabled] = useState(true);
  const [filtersChanged, setFiltersChanged] = useState(0);
  const searchParams = useSearchParams();
  const [searchQueries, setSearchQueries] = useState<UserFilterQueryNames>({
    isAdmin: searchParams.get("isAdmin") === "true" ? true : false,
  });
  ...
  useEffect(() => {
    ...
    for (const key in searchQueries) {
      if (searchQueries[key as keyof UserFilterQueryNames] !== null) {
        changedQueries += 1;
      }
    }
    changedQueries > 0
  ...
  const handleAddSearchFilters = () => {
    ...
    for (const key in searchQueries) {
      ...
      if (index !== -1) {
        if (searchQueries[key as keyof UserFilterQueryNames]) {
          params[index][1] = `${
            searchQueries[key as keyof UserFilterQueryNames]
          }`;
        } else {
...
export default PanelFilterUsers;
